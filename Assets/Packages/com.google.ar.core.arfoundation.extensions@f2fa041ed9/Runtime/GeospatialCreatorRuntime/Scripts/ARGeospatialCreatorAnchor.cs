...
#region Runtime-only anchor resolution methods
        ...
        private void AddGeoAnchorAtRuntime()
        {
            if (!ARCoreExtensions._instance)
            {
                ...
                {
                    Debug.LogWarning($"[ARGeospatialCreatorAnchor] {name}: ARCoreExtensions instance is null. Cannot create anchor.");
                }
                return;
            }
            ...
            if (sessionHandle == IntPtr.Zero)
            {
                if (Time.frameCount % 120 == 0) // Log every ~2 seconds
                {
                    Debug.Log($"[ARGeospatialCreatorAnchor] {name}: Session handle is zero. Waiting for ARCore session to initialize...");
                }
                return;
            }
            ...
            var trackingState = EarthApi.GetEarthTrackingState(sessionHandle);
            if (trackingState != TrackingState.Tracking)
            {
                if (Time.frameCount % 120 == 0) // Log every ~2 seconds
                {
                    Debug.Log($"[ARGeospatialCreatorAnchor] {name}: Waiting for AR Session to become stable. Current Earth tracking state: {trackingState}");
                }
                return;
            }
            ...
            {
                Debug.LogError($"[ARGeospatialCreatorAnchor] {name}: Unable to place anchor - AnchorManager property is null");
                _anchorResolution = AnchorResolutionState.Complete;
                return;
            }

            Debug.Log($"[ARGeospatialCreatorAnchor] {name}: AR Session is stable! Starting anchor resolution. AltitudeType: {this.AltitudeType}");
        ...
        private void FinishAnchor(ARGeospatialAnchor resolvedAnchor)
        {
            if (resolvedAnchor == null)
            {
                ...
                Debug.LogError($"[ARGeospatialCreatorAnchor] {name}: Failed to make Geospatial Anchor");
                _anchorResolution = AnchorResolutionState.Complete;
                return;
            }
            ...
            _anchorResolution = AnchorResolutionState.Complete;
            Debug.Log($"[ARGeospatialCreatorAnchor] {name}: Geospatial Anchor resolved successfully!");
        }
